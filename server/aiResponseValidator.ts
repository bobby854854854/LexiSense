...
export function validateAIResponse(data: any): {
  ...
  const insights: AIInsight[] = Array.isArray(data.insights) 
    ? data.insights.filter((insight: any) => 
  ...
  return {
    ...
    effectiveDate: typeof data.effectiveDate === 'string' ? data.effectiveDate : undefined,
    expiryDate: typeof data.expiryDate === 'string' ? data.expiryDate : undefined,
    riskLevel: ['low', 'medium', 'high'].includes(data.riskLevel) ? data.riskLevel : 'low'
  };
}